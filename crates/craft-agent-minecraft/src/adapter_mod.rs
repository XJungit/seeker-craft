...
impl MinecraftModAdapter {
    ...
    /// 采集控制器：Mod 侧自动找方块→导航→挖→收集。
    pub fn collect_start(&self, target: &str, count: u32) -> Result<ModAck> {
        self.lock_bridge()?.send(ModCommand::Collect {
            target: target.into(),
            count,
        })
    }
    ...
    /// 查询采集控制器状态。
    pub fn collect_status(&self) -> Result<ModAck> {
