                ...
                if t % 100 == 0 && t > 0 {
                    ...
                    if let Ok(inv) = bot.get_inventory() {
                        if let Some(slots) = inv.slots() {
                            let summary: Vec<String> = slots.iter().take(5).map(|s| {
                                if s.is_empty() { "空".to_string() } else { format!("{:?}", s) }
                            }).collect();
                    ...
                    match bot.nearby_players() {
                        Ok(players) => {
                            ...
                            let kinds: Vec<String> = players.iter().map(|p| {
                                match p.kind() {
                                    Ok(k) => format!("{:?}", k),
                                    Err(_) => "?".to_string(),
                                }
                            }).collect();
                            println!("[azalea_connect] t={t} 附近玩家实体数={count} kinds={kinds:?}");
                        }
