...
- **DSH 桥**：`/api/connect`（azalea 客户端连上 MC，账号 CraftAgent）/ `/api/bot_tool`
  （派发 49 工具之一，`GameTool::execute`）/ `/api/game-state`（实时 BotState 快照，
...
# 随后由 DSH 经 /api/connect 连接 bot；日志在 %TEMP%\opencode\viewer_run.log(.err)（可用 SEEKER_LOG_DIR 覆盖）
```
...
DSH 大脑 ──HTTP──► viewer（Axum）
  ...
  │  /api/bot_tool    → 派发 49 工具（GameTool::execute，含 P100/P101/P102/P132 自动修正）
