...
impl SceneFingerprint {
    ...
    pub fn from_scene(scene: &str, goal: &str) -> Self {
        ...
        for line in scene.lines() {
            if line.contains("HOTBAR") || line.contains("INVENTORY") {
                in_inv = true;
                in_nearby = false;
                continue;
            }
            if line.contains("NEARBY BLOCKS") {
                in_nearby = true;
                in_inv = false;
                continue;
            }
            if line.trim().is_empty() || line.starts_with("  Light") || line.starts_with("  Held") {
                continue;
            }
            if in_inv {
                inv_tokens.extend(tokenize(line));
            }
            if in_nearby {
                nearby_tokens.extend(tokenize(line));
            }
        }
        Self {
            goal_tokens,
    ...
    fn similarity(&self, other: &SceneFingerprint) -> (f64, f64, f64) {
        ...
        (
            jaccard(&self.goal_tokens, &other.goal_tokens),
            jaccard(&self.inv_tokens, &other.inv_tokens),
            jaccard(&self.nearby_tokens, &other.nearby_tokens),
        )
    }
}
...
impl Skill {
    ...
    fn mark_hit(&mut self, now_ms: i64) {
        self.hit_count += 1;
...
impl SkillLibrary {
    ...
    pub fn to_examples(
        ...
        self.match_scene(scene, goal, limit, now_ms)
            ...
            .map(|m| {
                if m.confidence > 0.5 {
                    m.skill.to_directive(m.confidence)
                } else {
                    m.skill.to_suggestion()
                }
            })
            .collect()
    }
    ...
    pub fn cleanup(&mut self, now_ms: i64, stale_ms: i64) {
        self.skills.retain(|s| {
            !(now_ms - s.last_used > stale_ms && (s.hit_count == 0 || s.success_count == 0))
        });
    }

    pub fn decay_all(&mut self, now_ms: i64, decay_ms: i64) {
        for s in &mut self.skills {
            s.decay(now_ms, decay_ms);
        }
    }
...
mod tests {
    ...
    fn fingerprint_similarity() {
        let f1 = SceneFingerprint::from_scene(
            "HOTBAR\n  [1] oak_planksx6\nNEARBY BLOCKS\n  oak_log at (-40,68,48)\n",
            "collect wood",
        );
        let f2 = SceneFingerprint::from_scene(
            "HOTBAR\n  [1] oak_planksx3\nNEARBY BLOCKS\n  oak_log at (-38,68,50)\n  birch_log",
            "collect wood",
        );
    ...
    fn scene_match_directive() {
        ...
        let fp = SceneFingerprint::from_scene(
            "HOTBAR\n  [1] oak_planksx4\nNEARBY BLOCKS\n  oak_log (-35,68,56)",
            "craft table",
        );
        lib.upsert(
            "craft crafting_table",
            vec!["craft".into(), "place".into()],
            vec!["craft".into()],
            fp,
        );
        let matches = lib.match_scene(
            "HOTBAR\n  [1] oak_planksx4\nNEARBY BLOCKS\n  oak_log",
            "craft table",
            5,
            1000,
