...
// Find everything after the system_prompt value
// Look for the next top-level key pattern: ,\\n  "key":
const afterPattern = /,\n  "[a-z_]+":/;
const afterMatch = raw.match(afterPattern);
let after = '';
if (afterMatch) {
  const afterIdx = raw.indexOf(afterMatch[0], startIdx + 100);
  if (afterIdx > 0) {
    after = raw.substring(afterIdx + 2);
  }
}

// New prompt - use proper JSON escaping
...
try {
  ...
  if (pos) {
    ...
    console.log('Context:', JSON.stringify(newFile.substring(Math.max(0,p-20), p+20)));
  }
}
