 ...
 *   - bot_tool(name, args)       POST /api/bot_tool    执行 49 个 Minecraft 工具之一（含 P100/P101/P102/P132 自动修正）
 ...
 *
 *   - {{tool_list}}   49 工具清单（ALL_TOOL_NAMES 静态镜像）——system 段变量，字节稳定
...
/** 49 工具清单（tools_azalea.rs::ALL_TOOL_NAMES 的静态镜像，稳定契约）。 */
const TOOL_NAMES = [
  ...
  'interact_entity', 'chat', 'memory', 'remember', 'run_plan', 'search_wiki',
  ...
  'chest_view', 'chest_withdraw', 'chest_deposit',
  'new_action', 'list_actions',
]
...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    name: 'bot_tool',
    description: '对 live bot 执行一个 Minecraft 工具（name 为 49 个已注册工具之一，见 persona 清单；args 为该工具参数对象）。' +
  ...
  //   POST /craft/api/bot_tool     → /api/bot_tool     执行 49 个 Minecraft 工具
