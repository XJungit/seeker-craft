...
// Read the current file to get all the non-prompt fields
const profilePath = 'D:\\Craft-Agent\\profiles\\_default.json';
const raw = fs.readFileSync(profilePath, 'utf8');

// Parse everything except the system_prompt
// First, extract all other fields by removing the system_prompt value
const startMarker = '"system_prompt": "';
const startIdx = raw.indexOf(startMarker);
const before = raw.substring(0, startIdx + startMarker.length);

// Find the end of the system_prompt value (the closing quote)
// We need to find the LAST " before the next , or }
let endIdx = -1;
for (let i = startIdx + startMarker.length; i < raw.length; i++) {
  if (raw[i] === '"' && raw[i-1] !== '\\') {
    // Check if this is followed by , or } or \n
    const next = raw[i+1];
    if (next === ',' || next === '}' || next === '\n' || next === undefined) {
      endIdx = i;
      break;
...
console.log('Before:', before.substring(before.length - 20));
console.log('After starts:', after.substring(0, 30));
...
// Reconstruct
const newFile = before + newPrompt + after;
fs.writeFileSync(profilePath, newFile);

// Verify
try {
  const j = JSON.parse(newFile);
