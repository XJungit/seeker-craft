...
// Read the current file to get the structure
const profilePath = 'D:\\Craft-Agent\\profiles\\_default.json';
const raw = fs.readFileSync(profilePath, 'utf8');

// Find everything before system_prompt
const startMarker = '"system_prompt": "';
const startIdx = raw.indexOf(startMarker);
const before = raw.substring(0, startIdx);

// Find everything after the system_prompt value (look for the next top-level key)
// The pattern is: "\n  "key_name":
let afterMatch = raw.match(/,\n  "[a-z_]+":/);
let after = '';
if (afterMatch) {
  const afterIdx = raw.indexOf(afterMatch[0], startIdx + 100); // skip ahead
  if (afterIdx > 0) {
    after = raw.substring(afterIdx + 2); // skip the comma and newline
  }
}

console.log('Before length:', before.length);
console.log('After starts with:', after.substring(0, 50));

// New prompt (properly escaped)
...
const newFile = before + '"' + newPrompt + '",\n' + after;
