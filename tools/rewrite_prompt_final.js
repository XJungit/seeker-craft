...
// Read the current file to extract non-prompt fields
const profilePath = 'D:\\Craft-Agent\\profiles\\_default.json';
const raw = fs.readFileSync(profilePath, 'utf8');

// Extract the modes field
const modesMatch = raw.match(/"modes":\s*\{[^}]+\}/);
const modes = modesMatch ? modesMatch[0] : '"modes": {"self_preservation": true, "self_defense": true, "unstuck": true}';

// Extract cooldown
const cooldownMatch = raw.match(/"cooldown":\s*(\d+)/);
const cooldown = cooldownMatch ? cooldownMatch[1] : '3000';

// Extract num_examples
const numExamplesMatch = raw.match(/"num_examples":\s*(\d+)/);
const numExamples = numExamplesMatch ? numExamplesMatch[1] : '3';

// Extract conversation_examples (simplified - just keep empty array)
const examples = '[]';
...
const newJson = {
  ...
  modes: JSON.parse(modes.replace('"modes": ', '')),
  conversation_examples: JSON.parse(examples)
};

fs.writeFileSync(profilePath, JSON.stringify(newJson, null, 2));
