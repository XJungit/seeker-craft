...
// Find the end of the string - look for the last " before ,\n or\n$
let valueEnd = raw.length - 1;
// Search backwards from end for the closing quote
for (let i = raw.length - 1; i > valueStart; i--) {
  if (raw[i] === '"' && raw[i-1] !== '\\') {
    // Check if this is followed by comma or newline or end
    const next = raw[i+1];
    if (next === ',' || next === '\n' || next === '}' || next === undefined) {
      valueEnd = i;
      break;
...
console.log('Found prompt from', valueStart, 'to', valueEnd);
console.log('Total file length:', raw.length);

// Extract just the first part (before system_prompt) and the last part (after system_prompt)
