...
3. 删除 onEndServerTick 方法
4. 删除 onServerTick 方法
5. 新增 scheduleMoveTick() 方法
6. 在 6 处 moveTarget 赋值后添加 scheduleMoveTick() 调用
"""
...
# 删除 ServerTickEvents.END_SERVER_TICK 注册
content = content.replace(
...
# 5) 在 onInitialize() 的 } 前插入 scheduleMoveTick() 方法
schedule_method = '''
...
# 6) 在 6 处 moveTarget = new double[]{...} 后添加 scheduleMoveTick()
targets_to_update = [
