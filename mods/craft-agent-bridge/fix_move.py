...
content = content.replace(
    ...
    '    /** 全局 MinecraftServer 引用。 */'
)

# 2) onInitializeServer -> onInitialize, remove server param in lambda
...
# 3) 替换移动系统：去掉 onEndServerTick + ServerTickEvents.END_SERVER_TICK
#    替换为 scheduleMoveTick() 方法

# 删除 onEndServerTick 方法
pattern = r'    private void onEndServerTick\(MinecraftServer server\) \{[^}]+\}'
# 需要处理嵌套括号，用更简单的方法
lines = content.split('\n')
new_lines = []
skip = False
...
for i, line in enumerate(lines):
    if 'private void onEndServerTick(MinecraftServer server)' in line:
        skip = True
        brace_depth = 0
        continue
    if skip:
...
# 替换 ServerTickEvents.END_SERVER_TICK 注册
content = content.replace(
    '        ServerTickEvents.START_SERVER_TICK.register(this::onStartServerTick);\n        ServerTickEvents.END_SERVER_TICK.register(this::onEndServerTick);',
    '        // 移动由 scheduleMoveTick() 通过 executeIfPossible 驱动'
)
content = content.replace(
    '        ServerTickEvents.END_SERVER_TICK.register(this::onEndServerTick);',
    '        // 移动由 scheduleMoveTick() 通过 executeIfPossible 驱动'
